...
export const CodeEditor = memo(function CodeEditor({ file, workspace, onAttachCode, onOpenFile, onContentChange, onAfterSave, onBreadcrumbClick, appLanguage = 'ru' }: Props) {
  ...
  const handleEditorMount = useCallback(
    (editor: editor.IStandaloneCodeEditor, monaco: typeof import('monaco-editor')) => {
      ...
      editor.addAction({
        id: 'editor.save-file',
        label: t('Сохранить файл', 'Save file'),
  ...
  const buildMenu = useCallback((): MenuItem[] => {
    ...
    if (sel) {
      ...
      if (onAttachCode) {
        ...
        if (range) {
          ...
          items.push({
            label: L === 'ru'
              ? `Прикрепить к чату (L${range.startLine}–${range.endLine})`
              : `Attach to chat (L${range.startLine}–${range.endLine})`,
            icon: '💬',
            action: () => {
              onAttachCode({
                filePath: file.path,
    ...
    items.push({
      label: t('Найти (Ctrl+F)', 'Find (Ctrl+F)'),
      icon: '🔎',
      shortcut: 'Ctrl+F',
    ...
    if (onAfterSave) {
      items.push({
        label: t('Сохранить (Ctrl+S)', 'Save (Ctrl+S)'),
        icon: '💾',
        shortcut: 'Ctrl+S',
    ...
    items.push({ label: '', separator: true, action: () => {} })
    items.push({
      label: t('Выделить всё', 'Select all'),
      icon: '▣',
      shortcut: 'Ctrl+A',
    ...
    items.push({ label: '', separator: true, action: () => {} })
    items.push({
      label: t('Копировать путь к файлу', 'Copy file path'),
      icon: '📋',
    ...
    <div className="flex flex-col h-full bg-[#0d1117]">
      ...
      <div className="flex-1 min-h-0 overflow-hidden" onContextMenu={handleContextMenu}>
        <Editor
          ...
          loading={<div className="flex items-center justify-center h-full bg-[#0d1117] text-zinc-500">{t('Загрузка редактора…', 'Loading editor…')}</div>}
        />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-0.5 px-4 py-1 bg-[#0d1117] border-t border-zinc-800/60 text-[11px] text-zinc-500 font-mono shrink-0">
        ...
        {onOpenFile && <span className="text-zinc-600 text-[10px]">{t('Ctrl+Click по импорту — открыть файл', 'Ctrl+Click on import — open file')}</span>}
        {TS_JS_LANGS.includes(file.language) && <span className="text-zinc-600 text-[10px]">{t('Ctrl+Click — подсказка', 'Ctrl+Click — tooltip')}</span>}
        {file.language === PYTHON_LANG && <span className="text-zinc-600 text-[10px]">{t('Ctrl+Click по импорту — в определение', 'Ctrl+Click on import — go to definition')}</span>}
        {onAfterSave && <span className="text-zinc-600 text-[10px]">{t('Ctrl+S — сохранить', 'Ctrl+S — save')}</span>}
