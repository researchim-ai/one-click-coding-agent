...
export const EditorTabs = memo(function EditorTabs({ files, activeFilePath, workspace, onSelect, onClose, onCloseAll, onCloseOthers, appLanguage = 'ru' }: Props) {
  ...
  const buildMenu = (file: OpenFile): MenuItem[] => {
    const items: MenuItem[] = [
      { label: t('Закрыть', 'Close'), icon: '×', shortcut: 'Ctrl+W', action: () => onClose(file.path) },
    ]
    if (onCloseOthers) {
      items.push({ label: t('Закрыть остальные', 'Close others'), icon: '⊘', action: () => onCloseOthers(file.path) })
    }
    if (onCloseAll) {
      items.push({ label: t('Закрыть все', 'Close all'), icon: '⊗', action: () => onCloseAll() })
    }
    items.push({ label: '', separator: true, action: () => {} })
    items.push({ label: t('Копировать путь', 'Copy path'), icon: '📋', action: () => window.api.copyToClipboard(file.path) })
    items.push({ label: t('Копировать относительный путь', 'Copy relative path'), icon: '📋', action: () => window.api.copyToClipboard(relPath(file.path)) })
    items.push({ label: t('Показать в проводнике', 'Reveal in explorer'), icon: '📂', action: () => window.api.revealInExplorer(file.path) })
    return items
  }
