    ...
    <div className="flex items-center bg-[#0d1117] border-b border-zinc-800/60 shrink-0 min-h-[33px]">
        ...
        {sorted.map((s) => {
          ...
          return (
            <button
              ...
              `}
            >
              <span className="max-w-[120px] truncate">{s.title || t('Новый чат', 'New chat')}</span>
              {s.messageCount > 0 && (
      ...
      <div className="flex items-center gap-0.5 px-1.5 shrink-0">
        <button
          ...
          title={t('Новая сессия', 'New session')}
        >
          +
        </button>
        <button
          ...
          title={t('Свернуть чат', 'Collapse chat')}
        >
          ▶
        </button>
      </div>
      ...
      {contextMenu && (
        ...
        >
          ...
          >
            {t('Удалить сессию', 'Delete session')}
          </button>
        </div>
      )}
    </div>
