<p align="center">
  <img src="build/icons/icon.svg" width="128" height="128" alt="One-Click Coding Agent">
</p>

<h1 align="center">One-Click Coding Agent</h1>

<p align="center">
  <strong>Полностью локальный AI-агент для кодинга</strong><br>
  Один клик — и у вас автономный ассистент, который читает, пишет и редактирует код, запускает команды, ищет по проекту.<br>
  Без облака, без API-ключей, без подписок.
</p>

<p align="center">
  <a href="https://github.com/researchim-ai/one-click-coding-agent/releases">Releases</a> ·
  <a href="https://github.com/researchim-ai/one-click-coding-agent/issues">Issues</a> ·
  <a href="https://github.com/researchim-ai">researchim-ai</a>
</p>

<p align="center">
  <img alt="License" src="https://img.shields.io/github/license/researchim-ai/one-click-coding-agent?color=blue">
  <img alt="GitHub release" src="https://img.shields.io/github/v/release/researchim-ai/one-click-coding-agent?include_prereleases">
  <img alt="Platform" src="https://img.shields.io/badge/platform-linux%20%7C%20macos%20%7C%20windows-brightgreen">
</p>
...
|-----------|---------|---------------|
| RAM | 24 ГБ (CPU) / 16 ГБ (hybrid) | 32+ ГБ |
| GPU VRAM | — (работает на CPU) | 24+ ГБ (NVIDIA/AMD/Apple Silicon) |
| Диск | 25 ГБ | 40 ГБ |
| ОС | Linux x64, macOS 12+, Windows 10+ | Ubuntu 22.04+, macOS 14+, Windows 11 |

### Пресеты контекста (Qwen3.5-35B-A3B ~19 ГБ)

Размер контекста подбирается автоматически по доступной RAM и VRAM.
KV-кэш квантизирован в q8\_0 (экономит ~2× памяти при минимальной потере качества).
Контекст рассчитывается по формуле KV bytes-per-token-per-layer, а не захардкожен.

| RAM | VRAM | Режим | Ожидаемый контекст |
|-----|------|-------|---------------------|
| 16 ГБ | 0 | — | 4 K (модель ~20 ГБ, тесно) |
| 24 ГБ | 0 | CPU | 16 K |
| 32 ГБ | 0 | CPU | 65 K |
| 48+ ГБ | 0 | CPU | 128 K |
| 16 ГБ | 8 ГБ | Hybrid | 32 K |
| 32 ГБ | 8 ГБ | Hybrid | 65 K |
| 48+ ГБ | 8 ГБ | Hybrid | 65 K |
| 16 ГБ | 12 ГБ | Hybrid | 16 K (RAM-bound) |
| 32 ГБ | 12 ГБ | Hybrid | 32 K |
| 48+ ГБ | 12 ГБ | Hybrid | 65 K |
| 16 ГБ | 16 ГБ | Hybrid | 32 K |
| 32 ГБ | 16 ГБ | Hybrid | 32 K |
| 48+ ГБ | 16 ГБ | Hybrid | 32 K |
| 32+ ГБ | 24 ГБ | Full GPU | 65 K |
| 48+ ГБ | 24 ГБ | Full GPU | 65 K |
| 32+ ГБ | 48+ ГБ | Full GPU | 128 K |

При достижении ~60% контекста автоматически срабатывает **суммаризация** — LLM сжимает старую историю в саммари, обеспечивая "бесконечный" контекст.

*Laptop GPU может давать меньше свободной VRAM — приложение закладывает запас.*
...
[Apache 2.0](LICENSE) — Copyright 2025-2026 [researchim-ai](https://github.com/researchim-ai)
